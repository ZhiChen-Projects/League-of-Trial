...
void Map::build()
{
    ...
    mLeftBoundary   = mOrigin.x; 
    mTopBoundary    = mOrigin.y;
    mRightBoundary  = mOrigin.x + (mMapColumns * mTileSize);
    mBottomBoundary = mOrigin.y + (mMapRows * mTileSize);
...
bool Map::isSolidTileAt(Vector2 position, float *xOverlap, float *yOverlap)
{
    ...
    *yOverlap = 0.0f;
    ...
    if (tile == 0 || tile == 25) return false;
